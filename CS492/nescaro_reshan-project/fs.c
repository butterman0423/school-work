...
void* fs_init(struct fuse_conn_info *conn)
{
	...
	inode_map = calloc(sb.inode_map_sz*FS_BLOCK_SIZE, 1); 
	if( disk->ops->read(disk, inode_map_base, sb.inode_map_sz, inode_map) )
		exit(1);

	// read block map
	block_map_base = inode_map_base + sb.inode_map_sz;
	block_map = calloc(sb.block_map_sz*FS_BLOCK_SIZE, 1); 
	if( disk->ops->read(disk, block_map_base, sb.block_map_sz, block_map) )
		exit(1);

	/* The inode data is in the next set of blocks */
	inode_base = block_map_base + sb.block_map_sz;
	n_inodes = (sb.inode_region_sz * FS_BLOCK_SIZE) / sizeof(struct fs_inode);
	inodes = calloc(n_inodes, sizeof(struct fs_inode));
	if (disk->ops->read(disk, inode_base, sb.inode_region_sz, inodes) )
		exit(1);
		    ...
		    struct fuse_file_info *fi)
{
	...
	if(inode->size < len_to_read)
		len_to_read = inode->size;
