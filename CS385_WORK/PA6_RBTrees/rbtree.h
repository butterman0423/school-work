...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
		...
		if(root_ == nullptr) {
			root_ = new Node(key, key_value.second);
			root_->color = BLACK;
			size_++;
			return;
		}

		bool isLeft = false; // last rot
		while(x != nullptr) {
			y = x; // Reset parent
			const K& xKey = x->key();	
			...
			isLeft = key < xKey;
			x = isLeft ? x->left : x->right;
		}
		
		// Insert at y
		Node<K, V> *newNode = new Node(key, key_value.second);
		newNode->parent = y;
...
private:
    ...
    void insert_fixup(Node<K, V> *z) {
		while(z->parent != nullptr && z->parent->color == RED) {
			Node<K, V> * father = z->parent;
			Node<K, V> * gfather = father->parent;

			bool isLeftChild = gfather->left == father;
			Node<K, V> * uncle = isLeftChild ? gfather->right : gfather->left;

			if( uncle != nullptr && uncle->color == RED ) {
				// Perform a color swap
				father->color = BLACK;
				uncle->color = BLACK;
				gfather->color = RED;
				z = gfather;
    ...
    void left_rotate(Node<K, V> *x) {
		...
		Node<K, V> * rTree = x->right;
		Node<K, V> * rleft = rTree->left;
    ...
    void right_rotate(Node<K, V> *x) {
        Node<K, V> * parent = x->parent;
		Node<K, V> * lTree = x->left;
		Node<K, V> * lright = lTree->right;
